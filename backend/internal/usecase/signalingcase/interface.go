...
type SignalingUseCaseInterface interface {
	...
	// 失敗した場合はエラーを返します。
	StartSession(ctx context.Context, roomID entity.RoomID, userID entity.UserID) error

	// LeaveSession は指定されたルームIDとユーザーIDのセッションを終了します。
	LeaveSession(ctx context.Context, roomID entity.RoomID, userID entity.UserID) error
